...
var ResourceSetListView = Backbone.View.extend({
	...
	load: function (callback) {
		...
		$.when(this.model.fetchIfNeeded({
				success: function (e) {
					$('#loading-resourcesets').addClass('label-success');
				}
			}),
...
var PolicyListView = Backbone.View.extend({
	...
	load: function (callback) {
		...
		$.when(this.model.fetchIfNeeded({
				success: function (e) {
					$('#loading-policies').addClass('label-success');
				}
			}),
			this.options.rs.fetchIfNeeded({
				success: function (e) {
					$('#loading-rs').addClass('label-success');
				}
			}),
			this.options.systemScopeList.fetchIfNeeded({
				success: function (e) {
...
ui.routes.push({
	path: "user/policy", name: "policy", callback:
		function () {
			...
			var view = new ResourceSetListView({
				model: this.resourceSetList,
...
ui.routes.push({
	path: "user/policy/:rsid", name: "editPolicies", callback:
		function (rsid) {
...
ui.routes.push({
	path: "user/policy/:rsid/new", name: "newPolicy", callback:
		function (rsid) {
...
ui.routes.push({
	path: "user/policy/:rsid/:pid", name: "editPolicy", callback:
		function (rsid, pid) {
			this.breadCrumbView.collection.reset();
			this.breadCrumbView.collection.add([
				{text: $.t('admin.home'), href: ""},
